...
- TWITTER_BRIDGE_URL -> api['base_url']
...
class PersonaConfig:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            and bool(self.target_audience and self.target_audience.strip())
            and bool(self.tone_of_voice and self.tone_of_voice.strip())
        )
...
class ScoringConfig:
    ...
    time_decay: Dict[str, int] = field(
        default_factory=lambda: {"half_life_hours": 24, "max_age_hours": 168}
    )
    author_modifiers: Dict[str, float] = field(
        default_factory=lambda: {
            "verified_boost": 1.2,
    ...
    content_modifiers: Dict[str, float] = field(
        default_factory=lambda: {
            "question_boost": 1.3,
            "media_boost": 1.1,
            "thread_boost": 1.15,
    ...
    priority_thresholds: Dict[str, float] = field(
        default_factory=lambda: {"high": 0.8, "medium": 0.5, "low": 0.2}
    )
    ...
    def validate_thresholds(self) -> bool:
        ...
        if "high" in thresholds and "medium" in thresholds and "low" in thresholds:
            if not (thresholds["high"] > thresholds["medium"] > thresholds["low"]):
                raise ValueError(
                    "Priority thresholds must be in descending order (high > medium > low)"
                )
        return True
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ScoringConfig":
        ...
        # Create temporary default instance to get default values
        defaults = cls()

        # Deep-merge provided sections over defaults
        metric_weights = defaults.metric_weights.copy()
        metric_weights.update(data.get("metric_weights", {}))

        time_decay = defaults.time_decay.copy()
        time_decay.update(data.get("time_decay", {}))

        author_modifiers = defaults.author_modifiers.copy()
        author_modifiers.update(data.get("author_modifiers", {}))

        content_modifiers = defaults.content_modifiers.copy()
        content_modifiers.update(data.get("content_modifiers", {}))

        priority_thresholds = defaults.priority_thresholds.copy()
        priority_thresholds.update(data.get("priority_thresholds", {}))

        return cls(
            metric_weights=metric_weights,
            time_decay=time_decay,
            author_modifiers=author_modifiers,
...
class AppConfig:
    ...
    def __post_init__(self):
        ...
        if self.persona is None:
            self.persona = PersonaConfig(
                name="DefaultBot", target_audience="general", tone_of_voice="neutral"
            )
    ...
    def validate(self) -> bool:
        ...
        try:
            if self.persona and not self.persona.is_valid():
    ...
    def apply_environment_overrides(self):
        ...
        if "BATCH_SIZE" in os.environ:
            try:
                ...
                pass
        if "TWITTER_BRIDGE_URL" in os.environ:
            self.api["base_url"] = os.environ["TWITTER_BRIDGE_URL"]
        if "LLM_PROVIDER" in os.environ:
            self.processing.setdefault("llm", {})["provider"] = os.environ[
                "LLM_PROVIDER"
            ]
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AppConfig":
        ...
        if (
            not persona_data.get("name")
            or not persona_data.get("target_audience")
            or not persona_data.get("tone_of_voice")
        ):
            if not persona_data:
        ...
        # Deep-merge provided sections over defaults
        api_config = temp_defaults.api.copy()
        api_config.update(data.get("api", data.get("app_config", {}).get("api", {})))

        logging_config = temp_defaults.logging.copy()
        logging_config.update(
            data.get("logging", data.get("app_config", {}).get("logging", {}))
        )

        processing_config = temp_defaults.processing.copy()
        processing_config.update(
            data.get("processing", data.get("app_config", {}).get("processing", {}))
        )
